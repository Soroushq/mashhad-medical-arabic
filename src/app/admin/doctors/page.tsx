        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden border">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-4 text-start text-xs font-semibold text-gray-600 uppercase tracking-wider">الطبيب</th>
                  <th className="px-6 py-4 text-start text-xs font-semibold text-gray-600 uppercase tracking-wider">التخصص</th>
                  <th className="px-6 py-4 text-start text-xs font-semibold text-gray-600 uppercase tracking-wider">الهاتف</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {doctors.map((doctor) => (
                  <tr key={doctor.id} className={`hover:bg-gray-50 transition ${!doctor.isActive ? 'opacity-50' : ''}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{doctor.titleAr}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-medium rounded-full ${doctor.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        ...
                        {canUserDelete && (
                          <>
                            <form action={toggleDoctorStatus} className="inline">
                              ...
                              <button type="submit" className="p-2 text-orange-600 hover:bg-orange-50 rounded-lg transition" title={doctor.isActive ? 'تعطيل' : 'تفعيل'}>
                                {doctor.isActive ? '🚫' : '✅'}
                              </button>
                            </form>
          ...
          {doctors.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mb-4">لا توجد أطباء مسجلين حالياً</p>
              {canUserEdit && (
                <Link
                  href="/admin/doctors/new"
